...
namespace DirectSoundLib
{
    ...
    public interface IDirectSoundCaptureBuffer8
    {
        ...
        /// <returns></returns>
        [PreserveSig]
        int Lock(uint dwOffset, uint dwBytes, out IntPtr ppvAudioPtr1, out int pdwAudioBytes1, out IntPtr ppvAudioPtr2, out int pdwAudioBytes2, int dwFlags);
        ...
        [PreserveSig]
        int GetObjectInPath(ref GUID rguidObject, int dwIndex, ref GUID rguidInterface, out IntPtr ppObject);

        [PreserveSig]
        int GetFXStatus(int dwEffectsCount, out int pdwFXStatus);
    }
}