...
DWORD WINAPI gplay_play_thread(LPVOID lpParam)
{
    ...
    {
        WaitForSingleObject(dsound->play_callback_evt, INFINITE);
		 ...
		 * 其他参数与DSCapture类似
		 */
        if((hret = dsound->lpDirectSoundBuffer8->lpVtbl->Lock(dsound->lpDirectSoundBuffer8, writeCursor, dsound->fmt.nAvgBytesPerSec, &dataPtr1, &dataLength1, &dataPtr2, &dataLength2, NULL)) != DS_OK)
        {
...
int gplay_dsound_open(void *ctx, const char *uri)
{
    ...
    LPDIRECTSOUNDBUFFER lpDirectSoundBuffer;     /* used for obtain LPDIRECTSOUNDBUFFER8 interface */
    LPDIRECTSOUNDBUFFER8 lpDirectSoundBuffer8;   
    WAVEFORMATEX fmt;
    DSBUFFERDESC bufdesc;
    /* used for play buffer notifycation */
    LPDIRECTSOUNDNOTIFY8 lpDirectSoundNotify8;
    HANDLE play_callback_evt;
    DSBPOSITIONNOTIFY dsBufferPosNotify;
    /* audio file */
    FILE *stream = NULL;
    /* diret sound context */
    gplay_dsound_ctx_t *dsound = (gplay_dsound_ctx_t*)ctx;
	...
	fmt.nChannels = 2;           /* 通道数量 */
	fmt.nSamplesPerSec = 44100; /* 采样率, 每秒采样次数 */
    /*
	 * 采样位数, 每个采样的位数
	...
	if(!(play_callback_evt = CreateEvent(NULL, 1, 0, NULL)))
    {
        gmedia_log_error("create notify evt failed, hret=%lld", hret);
        ret = ERR_DSOUND_ERR;
        goto end;
    }
    dsBufferPosNotify.dwOffset = bufdesc.dwBufferBytes - 1; // 当缓冲区播放完的时候进行通知
    dsBufferPosNotify.hEventNotify = play_callback_evt;
    if((hret = lpDirectSoundNotify8->lpVtbl->SetNotificationPositions(lpDirectSoundNotify8, 1, &dsBufferPosNotify)) != DS_OK)
    {
    ...
    dsound->play_callback_evt = play_callback_evt;
